...
Types are inferred in the following order:

1. **Explicit type annotations** in serializers (using `type` helper)
2. **Sorbet signatures** on serializer methods
3. **Sorbet signatures** on model methods
4. **ActiveRecord enums**
5. **SQL schema columns**
6. **TypeSpec model fallback** (using `typespec_from`)

#### Supported Sorbet Types

- `String`, `Integer`, `Float` → `string`, `int32`, `float64`
- `T::Boolean`, `TrueClass`, `FalseClass` → `boolean`
- `Date`, `DateTime`, `Time` → `plainDate`, `utcDateTime`
- `T.nilable(Type)` → `Type?` (optional)
- `T::Array[Type]` → `Type[]`
- Custom classes → Type references

#### RBI File Support

The integration also supports extracting types from RBI files generated by [Tapioca](https://github.com/Shopify/tapioca), enabling type inference from:
- External gem methods
- DSL-generated methods
- Any code with RBI type annotations

This works automatically when RBI files exist in `sorbet/rbi/` directory, with no configuration required.

### Type Declarations

Use the `type` helper to declare types for serializer attributes and methods. It accepts TypeSpec type symbols (`:string`, `:boolean`), plain Ruby classes (`String`, `Integer`), and Sorbet types (`T.nilable`, `T::Array`).
...
Wraps all generated models and routes in a TypeSpec namespace. This allows using TypeSpec reserved keywords (like `Model`, `Scalar`, `Enum`) as model names and automatically escapes conflicting field names with backticks.

Set to `nil` to disable (not recommended - will rename conflicting models with `_` suffix).
